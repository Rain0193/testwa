...
const { METHOD_MAP, ALL_COMMANDS } = require('appium-base-driver/build/lib/protocol/routes')
const { commands } = require('appium-android-driver/build/lib/commands/element')
...
METHOD_MAP['/wd/hub/session/:sessionId/element'] = {
    POST: { command: 'findElement', payloadParams: { required: ['using', 'value'], optional: ['mode', 'note'] } }
}
METHOD_MAP['/wd/hub/session/:sessionId/elements'] = {
    POST: { command: 'findElements', payloadParams: { required: ['using', 'value'], optional: ['mode'] } }
}
...
AndroidDriver.prototype.startAndroidSession = async function () {
    ...
    if (deviceLogPath) {
        ...
        this.logcatProcess = testwa.startLogcat(this.adb, deviceLogPath, this.sessionId);
    }
}
...
apkUtilsMethods.installFromDevicePath = async function (apkPathOnDevice, opts = {}) {
    await Promise.all([installFromDevicePath.bind(this)(apkPathOnDevice, opts), this.forceInstall()]);

}
apkUtilsMethods.install = async function (apk, replace = true, timeout = 60000) {
    await Promise.all([install.bind(this)(apk, replace, timeout), this.forceInstall()]);
...
apkUtilsMethods.forceInstall = async function () {
    let func = async function () {
        await sleep(2000);
        let element = await wd.findElOrEls("xpath", '//*[@content-desc="Install"]|//*[@text="Install"]', false);
        element = await wd.findElOrEls("xpath", '//*[@content-desc="CONFIRM"]|//*[@text="CONFIRM"]', false);
